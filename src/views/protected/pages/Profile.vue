...
<!--                </p>-->
            ...
            <div class="lg:w-9/12 md:w-9/12 w-full py-6 pl-6">
                ...
                <div class="w-8/12">
                    ...
                    <div class="mb-4">
                        <label class="text-medium font-normal text-gray-base mb-1">Contact number</label>
                        <input type="text" v-model="user.phoneNumbers" placeholder="Enter a region" class="py-3 px-5  text-medium rounded-md focus:outline-none border border-gray w-full">
                    </div>
                    <!--div class="mb-4">
                    ...
                    <div class="mb-4" v-show="user.role.type === 'Contract'">
...
export default {
    ...
    methods:{
        ...
        ...mapMutations(['updateLoader','profiledUser']),
        getProfile() {
            this.updateLoader(true);
            
                // const res = await this.fetchSingleUser({id: this.$store.state.auth.user.id});
                // axios.get(`${url}core/users/${payload.id}`,payload);
                const token = JSON.parse(localStorage.getItem('SEF_token'));
                axios.get(`${process.env.VUE_APP_URL}core/users/${this.$store.state.auth.user.id}`,{
                    headers: {
