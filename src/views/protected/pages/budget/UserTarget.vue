<template>
  <div class="">
    ...
    <div class="flex justify-between items-center px-8 mt-4">
      <h3 class="font-semibold">User target</h3>
      <div class="flex items-center ">
        <button v-show="createPermission || role === 'SUPER_ADMIN'" @click="cleanForm" class="py-2 px-4 text-medium border border-gray rounded-md focus:outline-none">
          Allocate target to user
        </button>
      </div>
    </div>
    <div class="py-3 px-6">
      ...
      <div class="px-4 bg-white rounded-sm py-5">
        ...
        <table class="table w-full">
          ...
          <tr class="border-b border-gray text-gray-base" v-for="target in targets" :key="target.id">
            <td class="py-4 text-medium px-3">{{ target.owner ? target.owner.firstName + ' '+ target.owner.lastName : '-'  }}</td>
            ...
            <td class="py-4 text-medium px-3">
              <button v-show="updatePermission || role === 'SUPER_ADMIN'" @click="edit(target)" class="text-blue-dark ml-4">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
    ...
    <vue-final-modal
        ...
        @closed="cancelTargetModal"
    >
      <div class="pb-3 border-b border-gray">
        <h5 >Allocate target to user</h5>
      </div>
      <form action=""
      >
        <div class="my-3">
          <label class="text-medium font-normal text-gray-base mb-2">Select a Territory User</label>
          <select v-model="form.ownerId" class="focus:outline-none text-medium py-2 px-4 w-full border border-gray rounded-md">
            <option>&nbsp;</option>
            <option v-for="role in users" :key="role.id" :value="role.id">{{ role.email }}</option>
          </select>
        </div>
        <div class="my-3">
        ...
        <div class="flex items-center justify-end">
          <button @click.prevent="cancelTargetModal" class="border border-blue-base py-2 px-8 text-medium rounded-md focus:outline-none mx-2">
            Cancel
          </button>
          <button @click.prevent="userState ? handleUpdate() : handleCreate()" :disabled="!activateButton" class="bg-blue-base py-3 px-5 text-white text-medium rounded-md focus:outline-none ml-4">
            <span v-show="!userState">
              Create
            </span>
            <span v-show="userState">
              Update
...
export default {
  ...
  computed:{
    ...
    updatePermission(){
      return PR.userTargetUpdatePermission(this.$store.state.auth.user.role.permissions);
    },
    createPermission(){
      return PR.userTargetCreatePermission(this.$store.state.auth.user.role.permissions);
    },
  },
  methods:{
    ...mapActions(['logout','fetchBudget','fetchRegion','getUserTargets','createTargetForUser','updateTargetForUser','fetchAllUsers','fetchUsersInTerritory']),
    ...
    async getTerritoryUser(page = 1,territory){
      this.updateLoader(true);
      try {
        const res = await this.fetchUsersInTerritory({page,id:territory});
        this.users = res.data
    ...
    async handleCreate(){
      ...
      try {
        ...
        this.form.territoryId = this.$route.params.territoryId;
        const res = await this.createTargetForUser(this.form);
        // console.log(res)
    ...
    async handleUpdate(){
      ...
      try {
        this.modal = false;
        const res = await this.updateTargetForUser(this.form);
        // console.log(res)
        this.$toast.success('Successfully created');
        this.fetchTargets();
        this.updateLoader(false)
    ...
    async fetchTargets(){
      this.updateLoader(true)
      try {
        const res = await this.getUserTargets({budgetId: this.$route.params.budgetId,territoryId: this.$route.params.territoryId})
  ...
  async mounted() {
    this.fetchTargets();
    // this.getRegion();
    this.getTerritoryUser(1,this.$route.params.territoryId);
    // this.getBudgets();
  }
}
</script>
