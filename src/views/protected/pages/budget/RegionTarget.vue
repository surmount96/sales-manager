...
<!--              <router-link :to="'/user/user/'+ (target.owner.regionManager !== null ? target.owner.regionManager.id : '')" class="hover:text-blue-100 hover:underline">-->
                {{ target.owner.regionManager ? target.owner.regionManager.firstName + ' '+target.owner.regionManager.lastName : '-'  }}
<!--              </router-link>-->
            </td>
            <td class="py-4 text-medium px-3">
              <router-link :to="'/user/region/'+ (target.owner !== null ? target.owner.id : '')" class="hover:text-blue-100 hover:underline">
                {{ target.owner.name }}
              </router-link>
            </td>
            ...
            <td class="py-4 text-medium px-3 flex items-center" >
              <router-link v-show="target.owner" :to="'/user/budgets/'+$route.params.id+'/regions/'+target.owner.id +'/territories/targets'" tag="button">
...
name: "RegionTarget",
  ...
  ...mapGetters(['role']),
    ...
    updatePermission(){
      return PR.regionTargetUpdatePermission(this.$store.state.auth.user.role.permissions);
    },
    createPermission(){
      return PR.regionTargetCreatePermission(this.$store.state.auth.user.role.permissions);
    },
    regionPermissionIndex(){
      return PR.regionIndexPermission(this.$store.state.auth.user.role.permissions);
    },
    regionTargetListPermission(){
      return PR.regionTargetIndexPermission(this.$store.state.auth.user.role.permissions);
    },
  },
  methods:{
    ...mapActions(['logout','fetchBudget','fetchRegion','getRegionTargets','createTargetForRegion','updateTargetForRegion','fetchAllUsers']),
