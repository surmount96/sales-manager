...
<!--        </button>-->
        ...
        <table class="table w-full">
          ...
          <tr class="border-b border-gray text-gray-base" v-for="target in targets" :key="target.id">
            ...
            <td class="py-4 text-medium px-3 flex items-center">
              <router-link v-show="target.owner" :to="'/user/budgets/'+$route.params.budgetId+'/territories/'+target.owner.id +'/users/targets'" tag="button">
    ...
    >
      <div class="pb-3 border-b border-gray">
        <h5 >Allocate target to territory</h5>
      </div>
      <form action=""
      >
        <div class="my-3">
    ...
    @closes="cancelTargetModal">
      <div class="">
        ...
        <button @click="cancelTargetModal" class="border border-blue-base py-2 px-8 text-medium rounded-md focus:outline-none mx-2">
          Cancel
        </button>
...
export default {
  ...
  computed:{
    ...
    updatePermission(){
      return PR.territoryTargetUpdatePermission(this.$store.state.auth.user.role.permissions);
    },
    createPermission(){
      return PR.territoryTargetCreatePermission(this.$store.state.auth.user.role.permissions);
    },
    targetListPermission(){
  ...
  async mounted() {
    this.fetchTargets();
    // this.getRegion();
    if(this.role === 'SUPER_ADMIN' || this.targetListPermission){
