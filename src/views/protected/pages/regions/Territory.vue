<template>
  <div>
    ...
    <div class="flex justify-end">
      <button @click="assignTerritoryUserModal = true" v-show="staffPermission || role === 'SUPER_ADMIN'" class="py-2 px-3 text-medium focus:outline-none border border-gray bg-white rounded-md mx-2 flex items-center">
        <i class="ri-dashboard-line mr-2 ri-fw"></i>
        Assign Staff to territory
      </button>

      <button @click="assignAgentModal = true" v-show="agentPermission || role === 'SUPER_ADMIN'" class="py-2 px-3 text-medium focus:outline-none border border-gray bg-white rounded-md mx-2 flex items-center">
        <i class="ri-dashboard-line mr-2 ri-fw"></i>
    ...
    <div class="bg-white p-4">
      <div class=" py-3 mb-4">
        <h4 class="text-large mb-3">
          <span class="text-medium font-semibold">Territory Sales Officer: {{ territory.territoryOfficer ? territory.territoryOfficer.firstName +' '+ territory.territoryOfficer.lastName : '-' }}</span>
        </h4>
        <p class="text-medium mb-3">
          <span class="font-semibold">Territory name: </span> {{ territory.name }}
        </p>
        <p class="text-medium">
          <span class="font-semibold">LGA(s): </span>
           <span v-for="(state,index) in territory.lgas" :key="state.id">
              {{ state.name }}{{ territory.lgas.length != (index+1) ? ', ': '' }}
          </span>
        </p>
      </div>
      <div class="">
        <h5 class="text-body-2 font-semibold mb-3"> Sales Staff</h5>
...
export default {
  ...
  computed:{
    ...mapGetters(['role']),
    staffPermission(){
      return PR.territoryMapStaffPermission(this.$store.state.auth.user.role.permissions)
    },
    agentPermission(){
      return PR.territoryMapAgentPermission(this.$store.state.auth.user.role.permissions)
    },
  },
  methods:{
    ...mapActions(['mapAgentToTerritory','fetchSingleTerritory','agentToStaffMapping','fetchAgents','fetchUsersInTerritory','mappingUserToTerritory','unMappedAgentsToTerritory']),
    ...
    async getSingleTerritory(){
      try{
        const res = await this.fetchSingleTerritory({id:this.$route.params.territoryId})
    ...
    async getUnmappedAgents(page = 1,data = '',){
      this.updateLoader(true);
      try {
        const res = await this.fetchAgents({roles:data,size:1000,page,partnerId:'', territoryId: ''});
