      ...
      <div class="">
        ...
        <table class="table-auto w-full shadow-sm">
          ...
          <tr class="border-b border-gray text-gray-base" v-for="user in users" :key="user.id">
            <td class="py-4 text-medium px-3">{{ user.firstName !== null ? user.firstName  : '-' }}</td>
            <td class="py-4 text-medium px-3"> {{ user.lastName !== null ? user.lastName  : '-' }}</td>
...
export default {
  ...
  methods:{
    ...mapActions(['logout','userInRegion']),
    ...
    async fetchUser(){
      ...
      try{
        const res = await this.userInRegion({id:this.$route.params.id,role:'ASS'})
        this.users =res.data
