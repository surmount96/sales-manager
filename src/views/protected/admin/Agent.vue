...
<!--          <td class="py-4 text-medium px-3 "></td>-->
        ...
        <tr class="border-b border-gray text-gray-base" v-for="user in users" :key="user.id">
          ...
          <td class="py-4 text-medium px-3">
            <svg @click="edit(user)" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-blue-dark" viewBox="0 0 20 20" fill="currentColor">
...
<!--          </td>-->
      ...
      <pagination v-show="users.length > 0" @pagePagination="getUsers" :meta="meta"/>
    </section>
...
export default {
  ...
  methods:{
    ...
    async getUsers(page = 1,data = '',){
      ...
      try {
        const res = await this.fetchAgents({roles:data,size:10,page,partnerId:this.search, territoryId: ''});
        // console.log(res)
        this.users = res.data
        this.meta = res.meta
